...
def parse_args():
    ...
    p.add_argument("--patience", type=int, default=0,
                   help="早停耐心值：val_acc 连续 N 个 epoch 未提升则停止训练；0 表示不启用早停")
    p.add_argument("--min_delta", type=float, default=0.0,
                   help="视为提升的最小 val_acc 增量；仅当 val_acc > best_acc + min_delta 才算改进")
    return p.parse_args()
...
def main():
    ...
    # 数据加载优化：num_workers>0 时开启 persistent_workers 与 prefetch_factor，
    # 避免每个 epoch 重新 fork worker 进程，并提前预取更多 batch，缓解 CPU 侧 fbank 提取瓶颈。
    _loader_extra = {}
    if args.num_workers and args.num_workers > 0:
        _loader_extra["persistent_workers"] = True
        _loader_extra["prefetch_factor"] = 4
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        if is_main_process(rank):
            ...
            if val_acc > best_acc + args.min_delta:
                ...
                # full_val_loader 一次性使用，不开 persistent_workers（会挂着 worker 进程）；
                # 只在 num_workers>0 时开 prefetch_factor 以加速一次性遍历。
                _fv_extra = {}
                if args.num_workers and args.num_workers > 0:
                    _fv_extra["prefetch_factor"] = 4
                ...
                no_improve += 1
                logger.info("no improvement: no_improve=%d/%d best_acc=%.4f",
                            no_improve, args.patience, best_acc)
        # 早停信号：只有 rank0 更新 no_improve，需要广播到所有 rank 一起退出，避免 DDP 卡死
        should_stop = torch.tensor([0], dtype=torch.int64, device=device)
        if is_main_process(rank) and args.patience > 0 and no_improve >= args.patience:
            should_stop[0] = 1
        if distributed:
        ...
        if should_stop.item() == 1:
            if is_main_process(rank):
                logger.info("early stopping triggered at epoch=%d (no_improve=%d >= patience=%d) best_acc=%.4f",
                            epoch, no_improve, args.patience, best_acc)
            break
    if is_main_process(rank):
        logger.info("done best_acc=%.4f output_dir=%s", best_acc, args.output_dir)
    if distributed:
        # 先释放训练期间残留的显存，给 NCCL 关闭留出必要的 CUDA 分配空间；
        # 再做一次 barrier 确保所有 rank 都走到清理点，避免个别 rank 提前退出触发通信异常。
        # destroy_process_group 只做进程组销毁，即便抛异常也不影响此前已保存的 best.pt / last.pt 等产物，
        # 因此这里吞掉异常并降级为 warning，避免让扫描器根据非零退出码把成功训练误判为失败。
        try:
            torch.cuda.empty_cache()
        except Exception as e:
            if is_main_process(rank):
                logger.warning("torch.cuda.empty_cache failed (non-fatal): %s", e)
        try:
            dist.barrier()
        except Exception as e:
            if is_main_process(rank):
                logger.warning("dist.barrier before destroy failed (non-fatal): %s", e)
        try:
            dist.destroy_process_group()
        except Exception as e:
            if is_main_process(rank):
                logger.warning("dist.destroy_process_group failed (non-fatal): %s", e)

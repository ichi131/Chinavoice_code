...
class TransformerDecoder(nn.Module):
    ...
    def forward(self, encoder_outputs, src_masks, padded_targets):
        """Teacher-forced decoder forward used for AED training."""
        tgt_mask = self.ignored_target_position_is_0(padded_targets, self.pad_id)
        dec_output = self.dropout(
            self.tgt_word_emb(padded_targets) * self.scale
            + self.positional_encoding(padded_targets)
        )
        for dec_layer in self.layer_stack:
            dec_output = dec_layer(
                dec_output, encoder_outputs, tgt_mask, src_masks
            )

...
class FireRedAsrAed(torch.nn.Module):
    ...
    def forward(self, padded_input, input_lengths, decoder_inputs):
        """Return AED logits and CTC log probabilities for joint training."""
        enc_outputs, enc_lengths, enc_mask = self.encoder(
            padded_input, input_lengths
        )
        decoder_logits = self.decoder(
            enc_outputs, enc_mask, decoder_inputs
        )
        ctc_log_probs = self.ctc(enc_outputs)
        return decoder_logits, ctc_log_probs, enc_lengths

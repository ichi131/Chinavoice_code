...
# -*- coding: utf-8 -*-
"""
prepare_mixft_v5_data.py
========================

混合方言联合微调 v5 数据准备脚本（去方言子标签版）。

与 v3 相比的唯一变化：
    * SFT target 的前缀改为官方推荐的 ``language Chinese<asr_text>`` 格式
      （不再包含具体方言名），目的是让模型只学中文 ASR 目标，不再显式
      预测方言标签，从而释放 CER 收益，避免方言互相混淆。
    * 对 VC_v2 已有 SFT 数据（其 text 字段本已带方言子标签）也会做重写。
    * 训练 jsonl 里仍保留 accent 字段，便于事后按方言做数据统计与
      分析；模型不会看到这个字段。
...
输出：
    - challenge_full_ft/data_mixft_v5/train.jsonl
    - challenge_full_ft/data_mixft_v5/val.jsonl
    - challenge_full_ft/data_mixft_v5/test.jsonl
    - challenge_full_ft/data_mixft_v5/stats.txt

用法：
    python challenge_full_ft/prepare_mixft_v5_data.py \
        --out_dir challenge_full_ft/data_mixft_v5 \
...
def build_target_text(accent: str, text: str) -> str:
    """构造 SFT 目标文本（v5：去掉方言子标签）。

    v3 原格式：``language Chinese {accent}<asr_text>{text}``
    v5 新格式：``language Chinese<asr_text>{text}``

    accent 参数保留但不再嵌入 target；调用方仍可用它做数据统计。
    """
    _ = accent  # keep signature compatible; not embedded in target anymore
    text = (text or "").strip()
    return f"language Chinese<asr_text>{text}"


def strip_prefix_to_text(raw_text: str) -> str:
    """从形如 ``language Chinese xxx<asr_text>YYY`` 中提取纯转写 YYY。

    - 若不含 ``<asr_text>``，视整串为纯转写直接返回（strip 后）。
    - 若含，则返回 ``<asr_text>`` 之后的部分（strip 后）。
    """
    s = (raw_text or "").strip()
    if ASR_MARKER in s:
...
def load_vc_v2_jsonl(path: str, tag: str) -> List[Dict]:
    """加载 VC_v2 SFT jsonl，并把 text 前缀重写为 v5 格式（去掉方言子标签）。

    VC_v2 原本的 text 字段形如：
        ``language Chinese anhui<asr_text>吃饭辰光到哉``
    v5 中会重写为：
        ``language Chinese<asr_text>吃饭辰光到哉``
    accent 字段保持原样。
    """
    if not os.path.isfile(path):
...
def parse_args() -> argparse.Namespace:
    ...
    p.add_argument("--out_dir", type=str,
                   default="/mnt/geminihzceph/user_johannapeng/challenge_model/challenge_full_ft/data_mixft_v5")

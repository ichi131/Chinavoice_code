#!/usr/bin/env python
# -*- coding: utf-8 -*-
"""
prepare_mixft_v6_data.py
========================

混合方言联合微调 v6 数据准备脚本
（在 v5 的基础上：对困难方言的 VC_v2 子集做 2× 重采样）。

与 v5 的唯一差异
----------------
* v5 输出的 target 格式保持不变：``language Chinese<asr_text>{文本}``
* 在合并 external + VC_v2 之后、全局 shuffle 之前，
  对以下"困难方言"的 **VC_v2 部分**（非 ext_ 开头的样本）在训练集中
  复制 1 份（即出现 2×），从而放大高质量真值样本对困难方言的贡献：
      hard_accents = {wuyu, kejia, chaoshan, nanchang, minnan}
  （依据 v5 by_dialect CER ≥ 20% 挑出）
* 外部方言数据（ext_*）不参与重采样，避免脏文本/风格不一致数据被放大。
* 验证集完全不变。

产物
----
    - challenge_full_ft/data_mixft_v6/train.jsonl
    - challenge_full_ft/data_mixft_v6/val.jsonl
    - challenge_full_ft/data_mixft_v6/test.jsonl
    - challenge_full_ft/data_mixft_v6/stats.txt

用法
----
    python challenge_full_ft/prepare_mixft_v6_data.py \\
        --out_dir challenge_full_ft/data_mixft_v6 \\
        --per_accent_max 25000 \\
        --hard_accents wuyu,kejia,chaoshan,nanchang,minnan \\
        --hard_dup 1 \\
        --seed 42
"""

from __future__ import annotations

import argparse
import json
import os
import random
import shutil
import sys
from collections import Counter, defaultdict
from typing import Dict, List, Tuple


# --------------------------- 配置区 --------------------------- #

EXT_ROOT = "/mnt/wfs/mmhuizhouwfssz/project_luban_infra/x_speech/user_johannapeng/challenge-data"

EXT_SOURCES: List[Tuple[str, str]] = [
    (f"{EXT_ROOT}/scripts/Kespeech_jsonl/hefei_anhui.jsonl",                                       "kespeech_anhui"),
    (f"{EXT_ROOT}/wenetspeech-yue/data_50h/meta_50h_converted.jsonl",                              "wenetspeech_yue"),
    (f"{EXT_ROOT}/Changsha_Dialect_Conversational_Speech_Corpus/changsha_segments_manifest.jsonl", "corpus_changsha"),
    (f"{EXT_ROOT}/scripts/Kespeech_jsonl/dongbei.jsonl",                                           "kespeech_dongbei"),
    (f"{EXT_ROOT}/scripts/Kespeech_jsonl/Henan.jsonl",                                             "kespeech_henan"),
    (f"{EXT_ROOT}/Zhengzhou_Dialect_Conversational_Speech_Corpus/Zhengzhou_segments_manifest.jsonl","corpus_zhengzhou"),
    (f"{EXT_ROOT}/Nanchang_Dialect_Conversational_Speech_Corpus/nanchang_segments_manifest.jsonl", "corpus_nanchang"),
    (f"{EXT_ROOT}/scripts/Kespeech_jsonl/Nanjing.jsonl",                                           "kespeech_nanjing"),
    (f"{EXT_ROOT}/scripts/Kespeech_jsonl/shan3xi.jsonl",                                           "kespeech_shan3xi"),
    (f"{EXT_ROOT}/scripts/Kespeech_jsonl/Shandong.jsonl",                                          "kespeech_shandong"),
    (f"{EXT_ROOT}/wenetspeech-chuan/data-50h/meta_50h_clean.jsonl",                                "wenetspeech_chuan"),
    (f"{EXT_ROOT}/Wuhan_Dialect_Scripted_Speech_Corpus/wuhan_dataset.jsonl",                       "corpus_wuhan"),
    (f"{EXT_ROOT}/wenetspeech-wu/audios_50h.jsonl",                                                "wenetspeech_wu"),
]

LABEL_TO_ACCENT: Dict[str, str] = {
    "Anhui":     "anhui",
    "Harbin":    "dongbei",
    "Shenyang":  "dongbei",
    "Dalian":    "dongbei",
    "Changchun": "dongbei",
    "Chaoyang":  "dongbei",
    "Chifeng":   "dongbei",
    "Dandong":   "dongbei",
    "Yingkou":   "dongbei",
    "Jilin":     "dongbei",
    "Liaoning":  "dongbei",
    "Zhengzhou": "henan",
    "Nanjing":   "nanjing",
    "XiAn":      "shan3xi",
    "Xian":      "shan3xi",
    "Jinan":     "shandong",
    "Qingdao":   "shandong",
    "Weihai":    "shandong",
    "Yantai":    "shandong",
    "cantonese": "cantonese",
    "Cantonese": "cantonese",
    "Changsha":  "changsha",
    "Henan":     "henan",
    "Nanchang":  "nanchang",
    "Sichuan":   "sichuan",
    "wuhan":     "wuhan",
    "Wuhan":     "wuhan",
    "Wuyu":      "wuyu",
    "wuyu":      "wuyu",
}

VC_V2_TRAIN = "/mnt/geminihzceph/user_johannapeng/challenge_model/challenge_full_ft/data_vc_v2/train.jsonl"
VC_V2_VAL   = "/mnt/geminihzceph/user_johannapeng/challenge_model/challenge_full_ft/data_vc_v2/val.jsonl"
VC_V2_TEST  = "/mnt/geminihzceph/user_johannapeng/challenge_model/challenge_full_ft/data_vc_v2/test.jsonl"


# --------------------------- 工具函数 --------------------------- #

ASR_MARKER = "<asr_text>"


def build_target_text(accent: str, text: str) -> str:
    """v6 目标文本格式（与 v5 一致）：language Chinese<asr_text>{text}"""
    _ = accent
    text = (text or "").strip()
    return f"language Chinese<asr_text>{text}"


def strip_prefix_to_text(raw_text: str) -> str:
    s = (raw_text or "").strip()
    if ASR_MARKER in s:
        return s.split(ASR_MARKER, 1)[1].strip()
    return s


def load_external_jsonl(path: str, source_tag: str,
                        unknown_label_counter: Counter) -> List[Dict]:
    if not os.path.isfile(path):
        print(f"[ERR] not found: {path}", file=sys.stderr)
        return []

    kept: List[Dict] = []
    n_line = n_skip_json = n_skip_label = n_skip_field = 0

    with open(path, "r", encoding="utf-8") as fin:
        for _line_no, raw in enumerate(fin, start=1):
            n_line += 1
            raw = raw.strip()
            if not raw:
                continue
            try:
                obj = json.loads(raw)
            except json.JSONDecodeError:
                n_skip_json += 1
                continue

            audio = obj.get("audio") or obj.get("wav_path") or ""
            text = obj.get("text") or ""
            label = obj.get("label") or obj.get("accent") or ""

            if not audio or not text:
                n_skip_field += 1
                continue

            accent = LABEL_TO_ACCENT.get(label)
            if accent is None:
                unknown_label_counter[f"{source_tag}::{label}"] += 1
                n_skip_label += 1
                continue

            key = f"ext_{source_tag}_{len(kept):07d}"
            kept.append({
                "audio":  audio,
                "text":   build_target_text(accent, text),
                "prompt": "",
                "key":    key,
                "accent": accent,
                "src":    source_tag,
            })

    print(f"[EXT] {source_tag:<20s} lines={n_line:>7d}  kept={len(kept):>7d}  "
          f"skip_json={n_skip_json}  skip_label={n_skip_label}  skip_field={n_skip_field}")
    return kept


def truncate_per_accent(samples: List[Dict], per_accent_max: int,
                        rng: random.Random) -> List[Dict]:
    by_accent: Dict[str, List[Dict]] = defaultdict(list)
    for s in samples:
        by_accent[s["accent"]].append(s)

    result: List[Dict] = []
    for accent, lst in sorted(by_accent.items()):
        if len(lst) > per_accent_max:
            rng.shuffle(lst)
            kept = lst[:per_accent_max]
            print(f"[TRUNC] {accent:<12s} {len(lst):>7d} -> {len(kept):>7d}")
        else:
            kept = lst
            print(f"[TRUNC] {accent:<12s} {len(lst):>7d} -> {len(kept):>7d}  (kept all)")
        result.extend(kept)
    return result


def load_vc_v2_jsonl(path: str, tag: str) -> List[Dict]:
    if not os.path.isfile(path):
        print(f"[ERR] VC_v2 file not found: {path}", file=sys.stderr)
        return []

    kept: List[Dict] = []
    n_rewritten = 0
    with open(path, "r", encoding="utf-8") as fin:
        for raw in fin:
            raw = raw.strip()
            if not raw:
                continue
            try:
                obj = json.loads(raw)
            except json.JSONDecodeError:
                continue

            accent = obj.get("accent", "")
            pure_text = strip_prefix_to_text(obj.get("text", ""))
            new_text = build_target_text(accent, pure_text)
            if new_text != obj.get("text", ""):
                n_rewritten += 1
            obj["text"] = new_text
            obj["src"] = tag
            kept.append(obj)
    print(f"[VC ] {tag:<20s} loaded={len(kept):>7d}  rewritten={n_rewritten}")
    return kept


def duplicate_hard_vc_samples(samples: List[Dict],
                              hard_accents: set,
                              hard_dup: int) -> Tuple[List[Dict], Counter]:
    """
    对困难方言的 VC_v2 子集做 hard_dup 次额外复制（总出现次数 = 1 + hard_dup 次）。
    只针对 key 不以 'ext_' 开头的样本；ext_* 样本不复制。

    - hard_dup=1 时相当于 2× 重采样（默认）。
    - 复制的样本内容与原样本完全一致（连 key 都一样），因为 SFT 训练不要求 key 唯一，
      而是保持样本级的语料重复，实现"权重放大"的效果。

    返回：(新的样本列表, 每个 accent 复制了多少条的计数)
    """
    dup_counter: Counter = Counter()
    out: List[Dict] = []
    for s in samples:
        out.append(s)
        if s["accent"] in hard_accents and not s["key"].startswith("ext_"):
            for _ in range(hard_dup):
                out.append(dict(s))  # 浅拷贝一份
                dup_counter[s["accent"]] += 1
    return out, dup_counter


def dump_jsonl(samples: List[Dict], path: str) -> None:
    os.makedirs(os.path.dirname(path), exist_ok=True)
    with open(path, "w", encoding="utf-8") as fout:
        for obj in samples:
            fout.write(json.dumps(obj, ensure_ascii=False) + "\n")


def accent_stats(samples: List[Dict]) -> Counter:
    return Counter(s["accent"] for s in samples)


def src_stats(samples: List[Dict]) -> Counter:
    return Counter(s.get("src", "?") for s in samples)


def format_stats(name: str, samples: List[Dict]) -> str:
    lines = [f"===== {name} (total={len(samples)}) ====="]
    lines.append("-- by accent --")
    for a, c in sorted(accent_stats(samples).items(), key=lambda x: (-x[1], x[0])):
        lines.append(f"  {a:<12s} {c}")
    lines.append("-- by source --")
    for s, c in sorted(src_stats(samples).items(), key=lambda x: (-x[1], x[0])):
        lines.append(f"  {s:<20s} {c}")
    return "\n".join(lines) + "\n"


# --------------------------- 主流程 --------------------------- #

def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser()
    p.add_argument("--out_dir", type=str,
                   default="/mnt/geminihzceph/user_johannapeng/challenge_model/challenge_full_ft/data_mixft_v6")
    p.add_argument("--per_accent_max", type=int, default=25000)
    p.add_argument("--hard_accents", type=str,
                   default="wuyu,kejia,chaoshan,nanchang,minnan",
                   help="逗号分隔的困难方言列表；这些方言的 VC_v2 子集会被 2× 重采样")
    p.add_argument("--hard_dup", type=int, default=1,
                   help="困难方言 VC_v2 子集额外复制多少次（=1 相当于 2×）")
    p.add_argument("--seed", type=int, default=42)
    return p.parse_args()


def main() -> None:
    args = parse_args()
    rng = random.Random(args.seed)
    out_dir = os.path.abspath(args.out_dir)
    os.makedirs(out_dir, exist_ok=True)

    hard_accents = {a.strip() for a in args.hard_accents.split(",") if a.strip()}
    print(f"\n[CFG] hard_accents = {sorted(hard_accents)}")
    print(f"[CFG] hard_dup     = {args.hard_dup} (total occurrences = {1 + args.hard_dup}x)")

    # ---------- 1. 加载全部外部方言 ---------- #
    print("\n[STEP 1] Loading external dialect jsonls ...")
    unknown_label_counter: Counter = Counter()
    ext_all: List[Dict] = []
    for path, tag in EXT_SOURCES:
        ext_all.extend(load_external_jsonl(path, tag, unknown_label_counter))

    if unknown_label_counter:
        print("\n[WARN] Unknown labels (skipped):")
        for k, v in unknown_label_counter.most_common():
            print(f"       - {k}: {v}")

    print(f"\n[STEP 1] external total loaded = {len(ext_all)}")

    # ---------- 2. 按 accent 采样截断 ---------- #
    print(f"\n[STEP 2] Truncating per-accent to <= {args.per_accent_max} ...")
    ext_trunc = truncate_per_accent(ext_all, args.per_accent_max, rng)
    print(f"[STEP 2] external after truncation = {len(ext_trunc)}")

    # ---------- 3. 加载 VC_v2 挑战集 SFT 数据 ---------- #
    print("\n[STEP 3] Loading VC_v2 challenge SFT data ...")
    vc_train = load_vc_v2_jsonl(VC_V2_TRAIN, "vc_v2_train")
    vc_val   = load_vc_v2_jsonl(VC_V2_VAL,   "vc_v2_val")

    # ---------- 4. 合并 ---------- #
    print("\n[STEP 4] Merging external + VC_v2 ...")
    train_merged = ext_trunc + vc_train
    print(f"[STEP 4] merged (before dup)  = {len(train_merged)}")

    # ---------- 5. 困难方言 VC_v2 子集重采样 ---------- #
    print(f"\n[STEP 5] Duplicating hard-dialect VC_v2 samples (dup={args.hard_dup}x extra) ...")
    train_dup, dup_counter = duplicate_hard_vc_samples(train_merged, hard_accents, args.hard_dup)
    print(f"[STEP 5] merged (after dup)   = {len(train_dup)}  (+{len(train_dup)-len(train_merged)})")
    for a in sorted(dup_counter.keys()):
        print(f"[STEP 5]   {a:<12s} added {dup_counter[a]} duplicates")

    # ---------- 6. Shuffle ---------- #
    print("\n[STEP 6] Shuffling final training list ...")
    rng.shuffle(train_dup)
    print(f"[STEP 6] final train samples = {len(train_dup)}")
    print(f"[STEP 6] final val   samples = {len(vc_val)}  (VC_v2 val, unchanged)")

    # ---------- 7. 写出 ---------- #
    print("\n[STEP 7] Writing jsonl ...")
    train_out = os.path.join(out_dir, "train.jsonl")
    val_out   = os.path.join(out_dir, "val.jsonl")
    test_out  = os.path.join(out_dir, "test.jsonl")
    stats_out = os.path.join(out_dir, "stats.txt")

    dump_jsonl(train_dup, train_out)
    dump_jsonl(vc_val,    val_out)

    if os.path.isfile(VC_V2_TEST):
        shutil.copyfile(VC_V2_TEST, test_out)
        print(f"[STEP 7] test.jsonl copied from {VC_V2_TEST}")
    else:
        with open(test_out, "w") as _:
            pass
        print("[STEP 7] test.jsonl written as empty placeholder")

    # ---------- 8. 统计 ---------- #
    stats_lines: List[str] = []
    stats_lines.append("### v6 config ###")
    stats_lines.append(f"hard_accents   = {sorted(hard_accents)}")
    stats_lines.append(f"hard_dup       = {args.hard_dup}  (occurrences = {1 + args.hard_dup}x)")
    stats_lines.append(f"per_accent_max = {args.per_accent_max}")
    stats_lines.append(f"seed           = {args.seed}")
    stats_lines.append("")
    stats_lines.append("### duplicates added per accent ###")
    for a in sorted(dup_counter.keys()):
        stats_lines.append(f"  {a:<12s} +{dup_counter[a]}")
    stats_lines.append("")
    stats_lines.append(format_stats("TRAIN", train_dup))
    stats_lines.append(format_stats("VAL",   vc_val))
    if unknown_label_counter:
        stats_lines.append("===== Unknown labels (skipped) =====")
        for k, v in unknown_label_counter.most_common():
            stats_lines.append(f"  {k}: {v}")
        stats_lines.append("")

    stats_text = "\n".join(stats_lines)
    with open(stats_out, "w", encoding="utf-8") as fout:
        fout.write(stats_text)

    print("\n" + stats_text)
    print(f"\n[DONE] outputs in: {out_dir}")


if __name__ == "__main__":
    main()
